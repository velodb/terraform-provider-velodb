---
page_title: "velodb_warehouses Data Source - velodb"
subcategory: ""
description: |-
  Lists VeloDB Cloud warehouses visible to the current organization, with optional filters for warehouse ID, name, cloud provider, region, deployment mode, and legacy keyword search.
---

# velodb_warehouses (Data Source)

Use the *velodb_warehouses* data source to list and filter warehouses in your VeloDB Cloud organization.

## Example Usage

### List All Warehouses

```terraform
data "velodb_warehouses" "all" {}

output "warehouse_count" {
  value = data.velodb_warehouses.all.total
}

output "warehouse_names" {
  value = [for wh in data.velodb_warehouses.all.warehouses : wh.name]
}
```

### Filter by Cloud Provider and Region

```terraform
data "velodb_warehouses" "beijing_saas" {
  cloud_provider  = "aliyun"
  region          = "cn-beijing"
  deployment_mode = "SaaS"
}
```

### Search by Keyword

```terraform
data "velodb_warehouses" "production" {
  keyword = "production"
}

output "prod_warehouse_ids" {
  value = [for wh in data.velodb_warehouses.production.warehouses : wh.warehouse_id]
}
```

## Enrichment behavior

`endpoint_service_id` and `endpoint_service_name` are not returned by the list
endpoint; the provider fills them per warehouse via the warehouse detail endpoint
on a best-effort basis. If that detail lookup is unavailable, the read still
succeeds with all other fields populated — those two fields may be empty, and the
provider surfaces a single aggregated warning (rather than one per warehouse).

<!-- schema generated by tfplugindocs -->
## Schema

### Optional

- `cloud_provider` (String) Cloud provider filter (e.g., `aws`, `aliyun`).
- `deployment_mode` (String) Deployment mode filter: `BYOC` or `SaaS`.
- `keyword` (String) Legacy local fuzzy match against warehouse name or exact ID.
- `name` (String) Partial warehouse name filter.
- `region` (String) Cloud region filter (e.g., `us-east-1`, `cn-beijing`).
- `warehouse_id` (String) Exact warehouse ID filter.

### Read-Only

- `total` (Number) Total number of matching warehouses.
- `warehouses` (Attributes List) List of matching warehouses. (see [below for nested schema](#nestedatt--warehouses))

<a id="nestedatt--warehouses"></a>
### Nested Schema for `warehouses`

Read-Only:

- `cloud_provider` (String) Cloud provider.
- `core_version` (String) Core version string.
- `created_at` (String) Creation time in ISO 8601 / RFC 3339 format.
- `deployment_mode` (String) Deployment mode: `BYOC` or `SaaS`.
- `endpoint_service_id` (String) PrivateLink endpoint service ID when available.
- `endpoint_service_name` (String) PrivateLink endpoint service name when available.
- `expire_time` (String) Expiration time when available.
- `name` (String) Warehouse display name.
- `pay_type` (String) Billing type: `PostPaid` or `PrePaid`.
- `region` (String) Cloud region.
- `status` (String) Current warehouse status.
- `warehouse_id` (String) Warehouse identifier.
- `zone` (String) Primary availability zone.
