...
- `ratio` (Number) vCPU-to-memory ratio (`1:4` or `1:8`). Supported values are `4` and `8`. This value can be set only when creating a cluster and does not control `cache_gb`.
